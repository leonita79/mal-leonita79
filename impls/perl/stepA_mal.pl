...
sub eval_defmacro_bang {
    ...
    my $key=$ast->[1];
    my $value=EVAL($ast->[2], $env);
    $value=do { my $fn=$value; bless sub { goto &$fn; }, 'MalMacro' } if ref $value eq 'CODE'; 
    $env->set($key=>$value);
    return $value;
}
sub eval_let_star {
...
sub eval_try_star {
    my ($ast, $env)=@_;
    my (undef, $try, $catch_block)=@$ast;
    return EVAL($try, $env) unless $catch_block; 
    ref $catch_block eq 'MalList' && ref $catch_block->[0] eq 'MalSymbol' && ${$catch_block->[0]} eq 'catch*' or die "bad catch* block\n";
    my (undef, $binding, $catch)=@$catch_block;
    eval {
        EVAL($try, $env);
    } // do {
        my $exception=$@;
        chomp $exception unless ref $exception;
        unless(ref $exception
              || Scalar::Util::looks_like_number($exception)
              || grep { $exception eq $_ } qw(true false nil)
        ) {
            my $string=$exception;
            $exception=bless \$string, 'MalString';
        }
        EVAL($catch, $env->extend($binding, $exception));
    };
}

my $line;
our $term=Term::ReadLine->new('mal');
my $prompt='user> ';
